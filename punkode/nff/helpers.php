            ...
            foreach (($set['versions'] ?? []) as $label => $v) {
                if (!empty($v['path']) && is_file($v['path'])) {
                    ...
                    if (!$res || empty($res['final']) || !is_file($res['final'])) {
                        ...
                        $ok2 = !empty($res2['final']) && is_file($res2['final']);
                        ...
                        // Solo se il fallback NON ha prodotto un file valido, metti l'errore
                        if (!$ok2) {
                            $res['error'] = $res2['error'] ?? ($res['error'] ?? 'save failed');
                        }
                    } else {
...
if (!function_exists('punk_process_upload')) {
    function punk_process_upload(array $files, array $sizes, array $opts = []): array
    {
        ...
        // 4)Crea Card per ogni file leggibili e usabili con tutti i dati
        $out = [];
        $nameMap = [];
        foreach ($files_organizzati_temp as $row) {
...
if (!function_exists('punk_slug_base')) {
    // EN: safe basename (no extension), ASCII-ish
    // IT: slug sicuro del nome base (senza estensione)
    function punk_slug_base(string $s): string
    {
        $s = preg_replace('~\.[^.]+$~', '', $s);                  // drop extension
        $s = preg_replace('~[^A-Za-z0-9._-]+~', '-', $s);         // non-safe → '-'
        $s = trim($s, '-._');
        return $s !== '' ? $s : 'image';
    }
}

if (!function_exists('punk_store_originals')) {
    /**
     * EN: Persist uploaded originals to a permanent folder (fast & robust).
     * IT: Salva gli originali caricati in una cartella permanente (veloce e robusto).
     *
     * $opts:
     *  - originals_dir        (string)  cartella base dove mettere gli originali (assoluta). Se vuota, prova: ($opts['dest_base'].'/originali') o __DIR__.'/originali'
     *  - group_path           (string)  per batch, es: "hd/pippo" → diventa sotto-cartelle degli originali
     *  - date_subdirs         (bool)    true=aggiunge Y/m/d (default true)
     *  - overwrite_originals  (bool)    false=evita sovrascrittura con -1, -2, ...
     *
     * RETURN: array di righe ok/errore, ciascuna tipo:
     *  [
     *    'ok'            => true,
     *    'original_path' => '/abs/.../originali/hd/pippo/2025/09/11/IMG_1234.jpg',
     ...
     *  ]
     */
    function punk_store_originals(array $files, array $opts = []): array
    {
        // 1) Normalizza upload (usa il tuo adapter esistente)
        try {
            $uploaded = punk_upload($files);
        } catch (\Throwable $e) {
            if (function_exists('punk_log')) punk_log('[store_originals][upload] ' . $e->getMessage(), 'error');
            return [['ok' => false, 'error' => 'upload_failed: ' . $e->getMessage()]];
        }

        // 2) Risolvi base dir per originali
        $base = rtrim((string)($opts['originals_dir'] ?? ''), "/\\");
        if ($base === '') {
            $destBase = rtrim((string)($opts['dest_base'] ?? ''), "/\\");
            $base = $destBase !== '' ? ($destBase . DIRECTORY_SEPARATOR . 'originali') : (__DIR__ . DIRECTORY_SEPARATOR . 'originali');
        }

        $groupPath     = trim((string)($opts['group_path'] ?? ''), "/\\"); // es. "hd/pippo"
        $dateSubdirs   = array_key_exists('date_subdirs', $opts) ? (bool)$opts['date_subdirs'] : true;
        $overwrite     = (bool)($opts['overwrite_originals'] ?? false);

        $out = [];

        // 3) Per ogni file, costruisci destinazione e sposta
        foreach ($uploaded as $row) {
            $tmp = $row['meta']['tmp_path'] ?? null;
            $on  = $row['name'] ?? ($row['meta']['safe_name'] ?? ($tmp ? basename((string)$tmp) : null));

            if (!$tmp || !is_file($tmp) || !$on) {
                $out[] = ['ok' => false, 'error' => 'missing_tmp_or_name'];
                continue;
            }

            // Base name + estensione originale (in minuscolo)
            $baseName = punk_slug_base($on);
            $ext = strtolower(pathinfo($on, PATHINFO_EXTENSION) ?: '');
            if ($ext === '') $ext = 'jpg';              // fallback prudente
            if ($ext === 'jpeg') $ext = 'jpg';

            // Costruisci la cartella di destinazione
            $dir = $base;
            if ($groupPath !== '') {
                $dir .= DIRECTORY_SEPARATOR . str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $groupPath);
            }
            if ($dateSubdirs) {
                $dir .= DIRECTORY_SEPARATOR . date('Y') . DIRECTORY_SEPARATOR . date('m') . DIRECTORY_SEPARATOR . date('d');
            }

            if (!is_dir($dir)) {
                if (!@mkdir($dir, 0775, true) && !is_dir($dir)) {
                    $out[] = ['ok' => false, 'error' => 'mkdir_failed: ' . $dir];
                    if (function_exists('punk_log')) punk_log('[store_originals] mkdir failed: ' . $dir, 'error');
                    continue;
                }
            }
            if (!is_writable($dir)) {
                $out[] = ['ok' => false, 'error' => 'not_writable: ' . $dir];
                if (function_exists('punk_log')) punk_log('[store_originals] not writable: ' . $dir, 'error');
                continue;
            }

            // Calcola nome finale + anti-collisione
            $dest = $dir . DIRECTORY_SEPARATOR . $baseName . '.' . $ext;
            if (!$overwrite) {
                $i = 1;
                while (is_file($dest)) {
                    $dest = $dir . DIRECTORY_SEPARATOR . $baseName . '-' . $i . '.' . $ext;
                    $i++;
                }
            }

            // Move robusto: rename → copy+unlink
            ...
            if (!$moved) {
                $err = error_get_last()['message'] ?? 'move_failed';
                $out[] = ['ok' => false, 'error' => $err];
                if (function_exists('punk_log')) punk_log('[store_originals] move failed: ' . $err, 'error');
                continue;
            }
            ...
            $hash  = is_readable($dest) ? ('sha256:' . hash_file('sha256', $dest)) : null;

            $out[] = [
                'ok'            => true,
                'original_path' => $dest,
                'original_name' => basename($dest),
                'orig_base'     => pathinfo($dest, PATHINFO_FILENAME),
                'bytes'         => $bytes,
                'mime'          => $mime,
                'sha256'        => $hash,
                'created_at'    => date('c'),
